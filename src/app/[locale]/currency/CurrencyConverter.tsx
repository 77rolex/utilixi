...
// Locale-priority currencies shown first in the rates table
const RATES_TOP: Record<string, string[]> = {
  en: ['USD', 'EUR', 'GBP', 'CAD', 'AUD', 'CHF', 'JPY', 'CNY'],
  ru: ['RUB', 'USD', 'EUR', 'UAH', 'BYN', 'KZT', 'CHF', 'CNY'],
  uk: ['UAH', 'USD', 'EUR', 'RUB', 'PLN', 'GBP', 'CHF', 'CZK'],
  fr: ['EUR', 'USD', 'GBP', 'CHF', 'CAD', 'MAD', 'DZD', 'TND'],
  lt: ['EUR', 'USD', 'GBP', 'PLN', 'SEK', 'DKK', 'NOK', 'CHF'],
};
...
export default function CurrencyConverter({ locale, rates, updatedAt }: Props) {
  ...
  const allRatesSorted = useMemo(() => {
    const top = (RATES_TOP[locale] || RATES_TOP.en).filter(c => c !== from && rates[c]);
    const rest = Object.keys(rates)
      .filter(c => c !== from && !top.includes(c))
      .sort((a, b) => a.localeCompare(b));
    return [...top, ...rest];
  }, [from, rates, locale]);

  const tableRows = useMemo(() => {
    const q = ratesQuery.trim().toLowerCase();
    const list = q
      ? allRatesSorted.filter(c => {
          const name = getCurrencyName(c, locale);
          return c.toLowerCase().includes(q) || name.toLowerCase().includes(q);
        })
      : allRatesSorted;
    return list.map(c => ({ code: c, rate: convert(1, from, c, rates) }));
  }, [allRatesSorted, ratesQuery, from, rates, locale]);
  ...
  return (
    <div className={styles['currency-widget']}>
      ...
      <div className={styles['currency-widget__table-wrap']}>
        <div className={styles['currency-widget__table-header']}>
          <p className={styles['currency-widget__table-title']}>
            {t.allRates} · 1 {from}
          </p>
          <input
            type="text"
            className={styles['currency-widget__table-search']}
