...
const META: Record<string, { title: string; description: string; h1: string }> = {
  ...
  ru: {
    title: 'Калькулятор вклада с капитализацией — расчёт депозита онлайн',
    description: 'Бесплатный калькулятор вклада онлайн. Рассчитайте итоговую сумму депозита с капитализацией процентов: ежемесячной, ежеквартальной или ежегодной. Сравните доход с простыми процентами.',
...
const CONTENT: Record<string, {
  ...
  ru: {
    description: 'Бесплатный калькулятор вклада показывает, как именно растут ваши сбережения с учётом капитализации процентов. Введите начальную сумму, годовую ставку и срок — выберите периодичность капитализации (ежемесячная, ежеквартальная, ежегодная или без неё) и получите итоговый баланс, сумму дохода и детальную разбивку по периодам.\n\nКапитализация процентов — главный инструмент роста вклада: проценты каждого периода прибавляются к основному телу, и уже на следующий период начисляются на большую сумму. Чем длиннее срок и выше ставка, тем значительнее разрыв между вкладом с капитализацией и простыми процентами. Используйте калькулятор для сравнения предложений банков перед открытием счёта.',
    ...
    faqs: [
      ...
      },
      {
        ...
        a: 'Формула сложных процентов: S = P × (1 + r/n)^(n×t), где P — начальная сумма, r — годовая ставка (дробью), n — количество капитализаций в год, t — срок в годах. Для простых процентов: S = P × (1 + r × t).',
      },
      {
      ...
      },
      {
        ...
        a: 'Нет, калькулятор показывает доход до налогообложения. В России действует налог на процентный доход по вкладам сверх необлагаемого минимума (ключевая ставка × 1 млн ₽). Уточняйте актуальные условия в вашем банке или налоговой инспекции.',
      },
      {
